...
- The adapter starts its lifecycle loop even when the configuration declares no MCP servers, so an installation whose every server arrives through runtime registration still gets idle cleanup and keep-alive recovery. Previously that path returned before starting the loop, which left runtime-registered servers with neither.
