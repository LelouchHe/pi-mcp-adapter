...
- A runtime registration wakes a deferred adapter. Upstream defers initialization at session start while a metadata cache is present, intending to wait for live metadata of configured servers; an installation with no configured servers would therefore stay uninitialized — no catalog, no auto-connect, and no native tools — until the first adapter tool call. The fork's runtime registration now starts the deferred runtime instead of waiting to be asked.
